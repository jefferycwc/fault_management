...
class TackerAPI():
    ...
    def create_vnf(self, vnf_name, vnfd_name, vim_name):
        ...
        print('create mongo successfully!!')
    ...
    def get_vnf_id(self,get_vnf_list_result,name):
        ...
        get_vnf_list_result = get_vnf_list_result['vnfs']
        for i in range(len(get_vnf_list_result)):
            if get_vnf_list_result[i]['description'] == 'mongo':
                return get_vnf_list_result[i]['id']
        return 0
        #return id
    def get_mongo_status(self,mongo_id):
        get_vnf_url = 'http://' + self.TACKER_IP + ':9890/v1.0/vnfs/' + str(mongo_id)
        token = self.get_token()
        headers = {'X-Auth-Token': token}
        get_vnf_response = requests.get(get_vnf_url, headers=headers)
        print("Get mongo status: " + str(get_vnf_response.status_code))
    ...
    def mongo_detect(self):
        ...
        if vnf_id==0:
            self.create_vnf('mongo','mongo','jefferyvim')
        #print('id: {}'.format(vnf_id))
        vnf_status = self.get_mongo_status(vnf_id) 
        if vnf_status!='200':
            self.create_vnf('mongo','mongo','jefferyvim')
        return 
        #print('mongo status: {}'.format(vnf_status))
    def list_ns(self):
...
class OpenStackAPI():
    ...
    def unpause_instance(self,instance_id):
        ...
        #print("resume mongo instance status: "+ str(res.status_code))
        #print(res)
        count=0
        while 1:
            if self.get_mongo_status(instance_id)=='ACTIVE':
                break
            time.sleep(1)
            count = count+1
            print('wait ' + str(count) + 's')
        print('unpause mongo successfully!!')
        #restart()
    ...
    def resume_instance(self,instance_id):
        ...
        while 1:
            ...
            count = count+1
            print('wait ' + str(count) + 's')
        print('resume mongo successfully!!')
        restart(instance_id)
    ...
    def reboot_instance(self,instance_id):
        ...
        while 1:
            ...
            count = count+1
            print('wait ' + str(count) + 's')
        print('reboot mongo successfully!!')
        restart(instance_id)
...
def restart(instance_id):
    ...
    #time.sleep(3)
    #test=OpenStackAPI()
    #status=test.get_mongo_status(instance_id)
    #while 1:
    #    print("the status is: {}".format(status))
    #    if status =='ACTIVE':
    #        break
    #    status=test.get_mongo_status()
    #time.sleep(10)
    print('resart mongo')
    count=0
    while 1:
    ...
    print('resart mongo successfully')
    return 
