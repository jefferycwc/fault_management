...
class TackerAPI():
    ...
    def get_vnf_id(self,get_vnf_list_result,name):
        ...
        get_vnf_list_result = get_vnf_list_result['vnfs']
        for i in range(len(get_vnf_list_result)):
            if get_vnf_list_result[i]['description'] == 'upf':
                return get_vnf_list_result[i]['id']
        return 0
        #return id
    def get_upf_status(self,upf_id):
        get_vnf_url = 'http://' + self.TACKER_IP + ':9890/v1.0/vnfs/' + str(upf_id)
        token = self.get_token()
        headers = {'X-Auth-Token': token}
        get_vnf_response = requests.get(get_vnf_url, headers=headers)
        print("Get upf status: " + str(get_vnf_response.status_code))
    ...
    def upf_detect(self):
        ...
        if vnf_id==0:
            self.create_vnf('upf','upf','jefferyvim')
        #print('id: {}'.format(vnf_id))
        vnf_status = self.get_upf_status(vnf_id) 
        if vnf_status!='200':
            self.create_vnf('upf','upf','jefferyvim')
        return 
        #print('upf status: {}'.format(vnf_status))
    def list_ns(self):
...
class OpenStackAPI():
    ...
    def unpause_instance(self,instance_id):
        ...
        #print("resume upf instance status: "+ str(res.status_code))
        #print(res)
        count=0
        while 1:
            if self.get_upf_status(instance_id)=='ACTIVE':
                break
            time.sleep(1)
            count = count+1
            print('wait ' + str(count) + 's')
        print('unpause upf successfully!!')
        #restart()
...
def restart(instance_id):
    ...
    #time.sleep(3)
    #test=OpenStackAPI()
    #status=test.get_upf_status(instance_id)
    #while 1:
    #    print("the status is: {}".format(status))
    #    if status =='ACTIVE':
    #        break
    #    status=test.get_upf_status()
    #time.sleep(10)
    print('resart upf')
    count=0
    while 1:
