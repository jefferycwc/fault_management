...
class TackerAPI():
    ...
    def get_vnf_id(self,get_vnf_list_result,name):
        ...
        get_vnf_list_result = get_vnf_list_result['vnfs']
        for i in range(len(get_vnf_list_result)):
            if get_vnf_list_result[i]['description'] == 'udm':
                return get_vnf_list_result[i]['id']
        return 0
        #return id
    def get_udm_status(self,udm_id):
        get_vnf_url = 'http://' + self.TACKER_IP + ':9890/v1.0/vnfs/' + str(udm_id)
        token = self.get_token()
        headers = {'X-Auth-Token': token}
        get_vnf_response = requests.get(get_vnf_url, headers=headers)
        print("Get udm status: " + str(get_vnf_response.status_code))
    ...
    def udm_detect(self):
        ...
        if vnf_id==0:
            self.create_vnf('udm','udm','jefferyvim')
        #print('id: {}'.format(vnf_id))
        vnf_status = self.get_udm_status(vnf_id) 
        if vnf_status!='200':
            self.create_vnf('udm','udm','jefferyvim')
        return 
        #print('udm status: {}'.format(vnf_status))
    def list_ns(self):
...
class OpenStackAPI():
    ...
    def unpause_instance(self,instance_id):
        ...
        #print("resume udm instance status: "+ str(res.status_code))
        #print(res)
        count=0
        while 1:
            if self.get_udm_status(instance_id)=='ACTIVE':
                break
            time.sleep(1)
            count = count+1
            print('wait ' + str(count) + 's')
        print('unpause udm successfully!!')
        #restart()
    ...
    def udm_detect(self):
        ...
        if udm_status=='PAUSED':
            self.unpause_instance(instance_id)
        elif udm_status=='SHUTOFF':
            self.reboot_instance(instance_id)
        elif udm_status=='SUSPENDED':
...
def restart(instance_id):
    ...
    #time.sleep(3)
    #test=OpenStackAPI()
    #status=test.get_udm_status(instance_id)
    #while 1:
    #    print("the status is: {}".format(status))
    #    if status =='ACTIVE':
    #        break
    #    status=test.get_udm_status()
    #time.sleep(10)
    print('resart udm')
    count=0
    while 1:
...
if __name__ == '__main__':
    ...
    while 1:
        test.udm_detect()
    #test = OpenStackAPI()
    #instance_id = test.get_instance_id('free5gc-udm-VNF')
    #restart(instance_id)
