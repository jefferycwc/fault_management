...
class OpenStackAPI():
    ...
    def unpause_instance(self,instance_id):
        ...
        #print("resume amf instance status: "+ str(res.status_code))
        #print(res)
        count=0
        while 1:
            if self.get_amf_status(instance_id)=='ACTIVE':
                break
            time.sleep(1)
            count = count+1
            print('wait ' + str(count) + 's')
        print('unpause amf successfully!!')
        #restart()
...
def restart(instance_id):
    ...
    #time.sleep(3)
    #test=OpenStackAPI()
    #status=test.get_amf_status(instance_id)
    #while 1:
    #    print("the status is: {}".format(status))
    #    if status =='ACTIVE':
    #        break
    #    status=test.get_amf_status()
    #time.sleep(10)
    print('resart AMF')
    count=0
    while 1:
