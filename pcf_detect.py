...
class TackerAPI():
    ...
    def get_vnf_id(self,get_vnf_list_result,name):
        ...
        get_vnf_list_result = get_vnf_list_result['vnfs']
        for i in range(len(get_vnf_list_result)):
            if get_vnf_list_result[i]['description'] == 'pcf':
                return get_vnf_list_result[i]['id']
        return 0
        #return id
    def get_pcf_status(self,pcf_id):
        get_vnf_url = 'http://' + self.TACKER_IP + ':9890/v1.0/vnfs/' + str(pcf_id)
        token = self.get_token()
        headers = {'X-Auth-Token': token}
        get_vnf_response = requests.get(get_vnf_url, headers=headers)
        print("Get pcf status: " + str(get_vnf_response.status_code))
    ...
    def pcf_detect(self):
        ...
        if vnf_id==0:
            self.create_vnf('pcf','pcf','jefferyvim')
        #print('id: {}'.format(vnf_id))
        vnf_status = self.get_pcf_status(vnf_id) 
        if vnf_status!='200':
            self.create_vnf('pcf','pcf','jefferyvim')
        return 
        #print('pcf status: {}'.format(vnf_status))
    def list_ns(self):
...
class OpenStackAPI():
    ...
    def unpause_instance(self,instance_id):
        ...
        #print("resume pcf instance status: "+ str(res.status_code))
        #print(res)
        count=0
        while 1:
            if self.get_pcf_status(instance_id)=='ACTIVE':
                break
            time.sleep(1)
            count = count+1
            print('wait ' + str(count) + 's')
        print('unpause pcf successfully!!')
        #restart()
...
def restart(instance_id):
    ...
    #time.sleep(3)
    #test=OpenStackAPI()
    #status=test.get_pcf_status(instance_id)
    #while 1:
    #    print("the status is: {}".format(status))
    #    if status =='ACTIVE':
    #        break
    #    status=test.get_pcf_status()
    #time.sleep(10)
    print('resart pcf')
    count=0
    while 1:
        ...
        if count==25:
            break
    #client.connect('172.24.4.105', 22,username='ubuntu',password='',pkey=key,compress=True)
    #stdin,stdout,stderr = client.exec_command('cd /home/ubuntu/stage3;sudo nohup ./bin/pcf & \n;exit')
