...
class OpenStackAPI():
    def __init__(self):
        ...
        self.project_id = ''
        self.lock = 0 
        self.channel_name = 'nssf_channel'
    def get_token(self):
        # print("\nGet token:")
        self.get_token_result = ''
    ...
    def nssf_detect(self):
        ...
        if nssf_status=='PAUSED' and self.lock==0:
            publisher(instance_id,'paused','nssf',self.channel_name)
            self.lock=1
        elif nssf_status=='SHUTOFF' and self.lock==0:
            publisher(instance_id,'shutoff','nssf',self.channel_name)
            self.lock=1
        elif nssf_status=='SUSPENDED' and self.lock==0:
            publisher(instance_id,'suspended','nssf',self.channel_name)
            self.lock=1
